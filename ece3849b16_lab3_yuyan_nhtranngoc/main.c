...
void initializeADC()
{
	...
	ADCSequenceStepConfigure(ADC0_BASE, 0, 1, ADC_CTL_CH0 ); // in the 0th step, sample channel 0
	ADCSequenceStepConfigure(ADC0_BASE, 0, 2, ADC_CTL_CH0 ); // in the 0th step, sample channel 0
	ADCSequenceStepConfigure(ADC0_BASE, 0, 3, ADC_CTL_CH0 | ADC_CTL_IE); // in the 0th step, sample channel 0
	ADCSequenceStepConfigure(ADC0_BASE, 0, 4, ADC_CTL_CH0 ); // in the 0th step, sample channel 0
	ADCSequenceStepConfigure(ADC0_BASE, 0, 5, ADC_CTL_CH0 ); // in the 0th step, sample channel 0
	ADCSequenceStepConfigure(ADC0_BASE, 0, 6, ADC_CTL_CH0 ); // in the 0th step, sample channel 0
	ADCSequenceStepConfigure(ADC0_BASE, 0, 7, ADC_CTL_CH0 | ADC_CTL_IE | ADC_CTL_END); // in the 0th step, sample channel 0
...
void ADC_ISR(void)
{
	...
	// Apparently this while loop messes up the readings.
	// No idea how to fix this. I'm too tired. Sorry Yigit :(
	// (tbh just comment the while loop out while getting sign off)
	while((ADC0_SSFIFO0_R & ADC_SSFSTAT0_EMPTY) != ADC_SSFSTAT0_EMPTY) {
		int buffer_index = ADC_BUFFER_WRAP(g_iADCBufferIndex + 1);
		g_psADCBuffer[buffer_index] = ADC_SSFIFO0_R & ADC_SSFIFO0_DATA_M; // read sample from the ADC sequence0 FIFO
...
void drawOverlay()
{
	...
	char freqString[20];							// Current frequency

	int marginVertical = 3;							// Text Alignment (Vertical Margin)
	int marginHorizontal = 5;						// Text Alignment (Horizontal Margin)
	int whole = g_frequency/1000;
	int decimal = g_frequency-whole*1000;
	// Draw frequency:
	usprintf(freqString, "%d.%dHz", whole, decimal);
	DrawString(0, 85, freqString, COLOR_TEXT, false);
